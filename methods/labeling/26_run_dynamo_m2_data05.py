...
def run_dynamo_m2(fold):
    ...
    dyn.pp.recipe_monocle(adata, n_top_genes=adata.n_vars, total_layers=False)
    ...
    # dyn.vf.VectorField(adata, vkey = 'dynamo_m2_velocity', basis=VIS_EMB.split('_')[1],map_topography=True, M=50,pot_curl_div=True)
    # dyn.tl.cell_velocities(adata, ekey='M_t', vkey='dynamo_velocity', basis=VIS_EMB.split('_')[1],enforce=True)
    # dyn.vf.VectorField(adata, basis=VIS_EMB.split('_')[1])
    # adata.obs['dynamo_time'] = adata.obs[f"{VIS_EMB.split('_')[1]}_ddhodge_potential"]
    if SAVE_DATA:
