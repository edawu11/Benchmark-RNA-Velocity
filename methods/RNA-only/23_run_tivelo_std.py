...
def run_tivelo_std(fold):
    ...
    adata_ = tivelo(adata, CLUSTER_KEY, VIS_EMB, data_name=DATASET, save_folder="results", njobs=8, tree_gene=None,
                show_fig=False, filter_genes=True, constrain=True, loss_fun="mse", only_s=False,
    ...
    adata_.layers["tivelo_std_velocity"] = adata_.layers.pop("velocity")

    
    if SAVE_DATA:
        adata_.write_h5ad(DATA_DIR / DATASET / "processed" / f"adata_run_tivelo_std_{fold}.h5ad")
    del adata_
    gc.collect()
