...
def run_velovae_fullvb(fold):
    adata = sc.read_h5ad(DATA_DIR / DATASET / "processed" / f"adata_preprocessed_{fold}.h5ad")
    full_vae = vv.VAE(adata, 
             tmax=20, 
             dim_z=5,
             full_vb=True,
             device='cuda:0')
    
    full_vae.train(adata, plot=False, embed=VIS_EMB.split('_')[1])
    full_vae.save_anndata(adata, 'velovae_fullvb', DATA_DIR / DATASET / 'processed', file_name=f"adata_run_velovae_fullvb_{fold}.h5ad")
