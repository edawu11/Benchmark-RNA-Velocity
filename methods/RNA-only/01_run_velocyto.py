...
def run_velocyto(fold):
    adata = sc.read_h5ad(DATA_DIR / DATASET / "processed" / f"adata_preprocessed_{fold}.h5ad")
    scv.tl.velocity(adata, mode="deterministic", vkey='velocyto_velocity')
    if SAVE_DATA:
        adata.write_h5ad(DATA_DIR / DATASET / "processed" / f"adata_run_velocyto_{fold}.h5ad")
    del adata
    gc.collect()
