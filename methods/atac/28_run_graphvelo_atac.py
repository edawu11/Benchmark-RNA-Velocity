...
def run_graphvelo_atac(fold):
    file_path = DATA_DIR / DATASET / "processed" / f"adata_run_multivelo_{fold}.h5ad"
    ...
    mack_score(adata, ekey='Ms', vkey='multivelo_velocity', tkey='multivelo_time',n_jobs=8)
    confident_genes = adata.var['mack_score'].sort_values(ascending=False)[:100].index
    gv = GraphVelo(adata, gene_subset=confident_genes, xkey='Ms', vkey='multivelo_velocity')
    gv.train()
    adata.layers['graphvelo_atac_velocity'] = gv.project_velocity(adata.layers['Ms'])
    ...
    adata.obs['graphvelo_atac_time'] = adata.obs[f"{VIS_EMB.split('_')[1]}_ddhodge_potential"]
    if SAVE_DATA:
        adata.write_h5ad(DATA_DIR / DATASET / "processed" / f"adata_run_graphvelo_atac_{fold}.h5ad")
    del adata  # Remove reference to AnnData object
    gc.collect()
if K_FOLD == 0:
    fold = 'full'
    run_graphvelo_atac(fold)
else:
