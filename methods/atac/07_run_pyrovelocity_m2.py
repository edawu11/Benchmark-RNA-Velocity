...
def run_pyrovelocity_m2(fold):
    ...
    adata.layers['pyrovelocity_m2_velocity'] = adata.layers.pop('velocity_pyro')
    cell_time = adata_model_pos[2]['cell_time']
    cell_time = np.squeeze(cell_time, axis = -1)
    cell_time_post_mean = np.mean(cell_time, axis = 0)
    adata.obs['pyrovelocity_m2_time'] = cell_time_post_mean
    if SAVE_DATA:
        adata.write_h5ad(DATA_DIR / DATASET / "processed" / f"adata_run_pyrovelocity_m2_{fold}.h5ad")
    del adata  # Remove reference to AnnData object
    gc.collect()

if K_FOLD == 0:
    fold = 'full'
    run_pyrovelocity_m2(fold)
else:
