...
# run unitvelo_independent
velo = utv.config.Configuration()
velo.R2_ADJUST = True
velo.IROOT = None
velo.FIT_OPTION = '2' # independent mode

def run_unitvelo_ind(fold, velo):
    ...
    adata.layers['unitvelo_ind_velocity'] = adata.layers.pop('velocity')
    adata.obs['unitvelo_ind_time'] = adata.obs['latent_time']
    if SAVE_DATA:
        adata.write_h5ad(DATA_DIR / DATASET / "processed" / f"adata_run_unitvelo_ind_{fold}.h5ad")
    del adata  # Remove reference to AnnData object
    gc.collect()

if K_FOLD == 0:
    fold = 'full'
    run_unitvelo_ind(fold, velo)
else:
